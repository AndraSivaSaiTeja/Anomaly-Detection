...
def evaluation(encoder, bn, decoder,m0,  m1, m2, m3, dataloader,device,_class_=None):
    ...
    with torch.no_grad():
        for img, gt, label, _ in dataloader:
            ...
            noisy_img = addnoise_batch(img,(-10,-10)) #use img,noisy_img for training M1,M2,M3 denoising modules
            ...
            inputs = encoder(noisy_img)
            #inputs = [tanh(m1(tanh(inputs[0]))),tanh(m2(tanh(inputs[1]))),tanh(m3(tanh(inputs[2])))] 
            outputs = decoder(bn(inputs))
            #outputs = [tanh(outputs[0]), tanh(outputs[1]), tanh(outputs[2])]
            #print(img.shape[-1])
