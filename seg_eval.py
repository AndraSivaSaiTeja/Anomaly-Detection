...
def norm_img(batch): #N*C*H*W,C=3
    
    for i in range(batch.shape[0]):
        img = batch[i]
        mean = [torch.mean(img[0]),torch.mean(img[1]),torch.mean(img[2])]
        eps = 0.1
        std = [torch.std(img[0])+eps,torch.std(img[1])+eps,torch.std(img[2])+eps]
        transform_norm = transforms.Compose([transforms.Normalize(mean=mean, std=std)])
        if(i==0):
            imgn = transform_norm(img).unsqueeze(dim=0)
        else:
            imgn = torch.concat((imgn,transform_norm(img).unsqueeze(dim=0)),dim=0)
...
def evaluation(model, dataloader,device,_class_=None):
    ...
    with torch.no_grad():
        for img, gt, label, _ in dataloader:
            ...
            outputs = model(transform_norm(img))
            #inputs = encoder(transform_norm(img))
            #inputs = [(m1((inputs[0]))),(m2((inputs[1]))),(m3((inputs[2])))] 
            ...
            anomaly_map = torch.nn.functional.sigmoid(outputs).cpu().squeeze(0)
            
            
            anomaly_map = gaussian_filter(anomaly_map, sigma=4) # Apply the gaussian filter
            #print(anomaly_map[np.newaxis,:,:].shape)
            gt[gt > 0.5] = 1
            gt[gt <= 0.5] = 0
            if label.item()!=0:
                aupro_list.append(compute_pro(gt.squeeze(0).cpu().numpy().astype(int),
                                              anomaly_map[np.newaxis,:,:].squeeze(0)))
