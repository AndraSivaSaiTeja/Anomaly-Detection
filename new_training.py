...
import argparse
from new_test import evaluation, visualization, test
...
def train(_class_):
   ...
   # print(list(encoder.children()))
    en_blocks = list(encoder.children())
    encoder1 = torch.nn.Sequential(*en_blocks[0:5])
    encoder2 = torch.nn.Sequential(*en_blocks[5:6])
    encoder3 = torch.nn.Sequential(*en_blocks[6:7])
   # print(list(encoder1.children()))
    #print(list(encoder2.children()))
    #print(list(encoder3.children()))
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(dataloader):
           ...
           # print("img=", torch.max(img0), torch.min(img0), img0.dtype)
            ...
            e3 = encoder3(e2)
            cli = m0(noisy_img)
            #clf1 = tanh(m1(tanh(encoder1(cli)))
            #clf2 = m2(tanh(encoder2(clf1)))
            #clf3 = m3(tanh(encoder3(clf2)))
            nf1 = encoder1(cli)
            nf2 = encoder2(nf1)
            nf3 = encoder3(nf2)
            loss0 = torch.clip(loss_function1(img.detach(),cli.detach())/(256*256),0,1e+15)
            loss1 = torch.clip(loss_function1((e1.detach()),(m1((nf1.detach()))))/(256*256),0,1e+15)
            loss2 = torch.clip(loss_function1((e2.detach()),(m2((nf2.detach()))))/(256*256),0,1e+15)
            loss3 = torch.clip(loss_function1((e3.detach()),(m3((nf3.detach()))))/(256*256),0,1e+15)
            
            optimizer0.zero_grad()
            loss0.requires_grad_()
            loss0.backward()
            optimizer0.step()
        ...
        if (epoch + 1) % 1 == 0:
            auroc_px, auroc_sp, aupro_px = evaluation(encoder1,encoder2,encoder3, bn, decoder, m0, m1, m2, m3, test_dataloader, device)
