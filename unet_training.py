...
#from torchvision import segmentation_models_pytorch as smp
import torch
...
import argparse
from unet_test import evaluation, visualization, test
...
def train(_class_):
    ...
    m0 = ResNetUNet(3,3)
    m1 = Dn1() # ResNetUNet(64,64)
    m2 = Dn2() #ResNetUNet(128,128)
    m3 = Dn3() #ResNetUNet(256,256)
    m0 = m0.to(device)
    m1 = m1.to(device)
    m2 = m2.to(device)
    m3 = m3.to(device)
    ...
    optimizer0 = torch.optim.Adam(list(m0.parameters()), lr=learning_rate, betas=(0.5,0.999))
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(dataloader):
           ...
           # print("img=", torch.max(img0), torch.min(img0), img0.dtype)
            ...
            loss0 = loss_function(img.detach(),m0(noisy_img.detach()))
            loss1 = loss_function(tanh(clear_fmaps[0].detach()),tanh(m1(tanh(noisy_fmaps[0].detach()))))
            loss2 = loss_function(tanh(clear_fmaps[1].detach()),tanh(m2(tanh(noisy_fmaps[1].detach()))))
            loss3 = loss_function(tanh(clear_fmaps[2].detach()),tanh(m3(tanh(noisy_fmaps[2].detach()))))
